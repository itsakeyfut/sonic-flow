//! Enhanced UI binding with integrated audio support
//!
//! This module extends the existing MainWindowBinding with audio integration
//! using event loop based architecture with safe channel communication.
...
use crate::ui::audio_integration::{AudioCommand, AudioIntegration};
use crate::ui::bindings::MainWindow; // Use existing MainWindow
...
/// Enhanced UI binding with audio integration
pub struct EnhancedMainWindowBinding {
...
impl EnhancedMainWindowBinding {
    /// Create a new enhanced main window binding
    pub fn new() -> Result<Self> {
        info!("Creating enhanced main window binding");

        let window = MainWindow::new().map_err(|e| {
            crate::error::Error::Application(format!("Failed to create main window: {}", e))
        })?;
        ...
        info!("Enhanced main window binding created successfully");
        Ok(binding)
    }
    ...
    pub fn run(&self) -> Result<()> {
        ...
        self.window.run().map_err(|e| {
            crate::error::Error::Application(format!("UI main loop error: {}", e))
        })
    }
